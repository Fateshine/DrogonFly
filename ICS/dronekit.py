...
from ICS.dronekit import connect, VehicleMode, LocationGlobal, LocationGlobalRelative,Locations,LocationLocal
...
def fly(waypoint,speed):
    ...
    mydb = client["Command"]
    global drone_sim,collection_sim
    collection_sim=mydb.sim
    collection_drone=mydb.WPS
    drone_WPS = pd.DataFrame(list(collection_drone.find())).drop_duplicates(subset=['Drone','x','y','z'])
    ...
    #     time.sleep(0.1)
    for i in range(drone_sim["Drone"].idxmax()+1):
            drone_tmp=drone_sim.loc[drone_sim['Drone'] == i].reset_index()
            point_last=[0,5,0]
            for j,point in drone_tmp.itertuples():
                if j==0:
                    continue
                goto(point[0]-point_last[0], point[1]-point_last[1], point[2]-point_last[2],speed,i)
                point_last=point
